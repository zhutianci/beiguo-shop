 ...
 *   · 「兑换码 / 充值卡 / 卡密」有真实联想，但**这一页的商品不发卡密**（交付方式是 SMS：
 *     付款后 lib/vmq.ts fulfillOrder → lib/sms.ts acquireForOrder 由服务端自动取号，
 *     号码和验证码直接显示在订单里）。这几个词只在把读者导向充值类商品时出现。
 *     2026-09-24 修正：原文有五处写成「交付的是卡密」「一张卡密对应一次收码」，
 *     其中两处进了 FAQPage 结构化数据；第二十四节 §8② 记着这一类错误已修，实际这一页漏了。
 ...
 * 长期算下来花得更多，把这笔账摊开给他看，比多卖一单 15 块的接码值钱。
 */
...
const FAQS: { q: string; a: string }[] = [
  {
    q: 'codex 接码是什么意思？',
    a: '接码就是借一个能收短信的手机号，用它接收一条一次性的验证码，收完这一次就结束。号码不归你，也不是买号、租号或者共享账号。注册 OpenAI Codex 用的是 ChatGPT 账号，如果注册过程里卡在「请输入手机号码」那一步、国内号码又过不去，用得上的就是这个。本站这一类不发卡密：付款后系统自动为这一单取号，号码和验证码直接显示在订单里，账号全程在你手里。',
  },
  {
  ...
  {
    q: '一单能接几次码？',
    a: '美区实体手机卡那一档的商品名里写明是「单次接码」，一单对应一次收码，要再收一次得再下一单。随机地区那一档如果你需要多次使用，下单前请找客服确认。',
  },
  {
    q: '号码收不到验证码，钱退不退？',
    a: '美区实体手机卡那一档商品页写的是「包过，不成功不收费，有问题及时联系客服」，按这个口径处理，遇到问题直接找客服。系统这一侧的规则是：号码取出后迟迟收不到码，可以在订单里换号重试；等到超时仍未收到验证码的，系统会取消这个号码并把订单标记为退款处理；验证码已经收到、这一单已经完成的，按已使用处理，退不了。号码一取出就开始计时，所以建议下单之前先把注册页面准备到手机验证那一步，别拿到号码再回头研究下一步该点哪里。',
  },
  {
  ...
  },
  {
    ...
    a: '搜「codex 接码 linux do」或者「代充 靠谱吗」的人，都是在找有人用过的证据，这种谨慎是对的。可以核验的有几条：本站由益阳市赫山区必高科技有限公司运营，能开具增值税发票（标价不含税，开票需另付 6% 税费）；价格、库存、累计成交数直接显示在本页的表里，成交数是真实订单数；付款后系统自动取号，号码和验证码显示在你自己的订单里，不需要你交出任何账号密码；退款与质保规则公开写在服务条款里。最便宜的一档只要几块钱，不放心就先花这个钱把流程跑通一次再说。',
  },
  {
    ...
    <>
      ...
      >
          ...
          <PriceTable
            ...
            note={
              <>
                ...
                美区实体手机卡那一档的商品名里写明是「单次接码」，一单对应一次收码；
        ...
        <Section id="physical-vs-virtual" heading="美区实体手机卡 vs 随机地区：钱到底花在哪">
          ...
          <Warning>
            便宜档失败一次，损失的不只是那几块钱。
            <strong>号码被拒或一直收不到码，就得换号、甚至重新走一遍注册流程</strong>
            ——超时仍未收到验证码的，系统会取消这个号码并把订单标记为退款处理，钱不会白扣，
            但耽误的时间回不来（美区实体卡那一档另有商品页写明的包过口径，见上表）。
        ...
        <Section id="troubleshoot" heading="收不到验证码：按这个顺序排查">
          ...
          <p className="pt-2">
            另外两件不要做的事：不要把订单信息或者拿到的号码贴到公开论坛里求助，
        ...
        <Section id="platform" heading="为什么不自己去接码平台买">
          ...
          <CheckList
            items={[
              ...
              <>
                <strong className="text-white/80">号码直接显示在你的订单里，不碰你的账号</strong>
                ：付款后系统自动取号，注册动作从头到尾是你自己做的，
                我们不需要、也不会要你的账号密码。
              </>,
              <>
