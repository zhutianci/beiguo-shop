...
export async function GET() {
  try {
    ...
    const [me, products, myPrices, rewards] = await Promise.all([
...
export async function POST(request: NextRequest) {
  try {
    ...
    const baseMap = new Map(products.map((p) => [p.id, Number(p.price)]))

    for (const item of parsed.data.prices) {
      const base = baseMap.get(item.productId)
      ...
      if (item.price < base) {
        return error(`专属价不能低于基础售价（¥${base.toFixed(2)}）`)
      }
