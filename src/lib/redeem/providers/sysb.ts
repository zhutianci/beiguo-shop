      ...
      if (route === 'v1') {
        ...
        return redeemCardV1({
          ...
          markIrreversible: async () => {
            ...
            if (!ok) {
              /*
               * 【这句话以前是错的，2026-09-19 改】原文写死
               * 「这张卡密已经提交过一次充值，正在处理中」。
               *
               * 但抢不到锁只能证明「15 分钟内有过一次提交尝试」，
               * **证明不了那次真的提交到了上游**。最常见的情况恰恰相反：
               * 上一次在 precheckAccount 就被上游拒了（凭据有效期不足之类），
               * 什么都没提交，可锁已经落下。买家重新取 Session 再来，
               * 看到的却是「正在处理中」——于是合理地认为是本站故障。
               *
               * 好在 hit 就在同一个作用域里（上面已经查过上游），
               * 不用再发一次请求就能把真实状态说出来。
               */
              const untouched = hit?.status === 'UNUSED' || hit?.status === 'RETRYABLE'
              throw new RedeemError(
                untouched
                  ? '这张卡密在 15 分钟内有过一次提交尝试，为防止重复扣卡被暂时锁定。' +
                    '**经向上游核对，这张卡密仍未被消耗**——上一次多半没有真正提交成功' +
                    '（最常见的原因是登录凭据已过期）。请重新获取一份 Session，稍后再提交一次。'
                  : '这张卡密在 15 分钟内有过一次提交尝试，为防止重复扣卡被暂时锁定。' +
                    '请先点「查询」确认上一次的真实结果，不要直接重复提交。',
                'PROCESSING',
                true
              )
            }
